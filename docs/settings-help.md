...
- Web 设置页不直接暴露 `SCHEDULE_TIME` / `SCHEDULE_TIMES` / `SCHEDULE_SLOTS` / `SCHEDULE_RUN_IMMEDIATELY` 等内部键；用户通过“定时任务”卡片维护启用状态、时间+市场行和立即执行一次。
...
- `SCHEDULE_SLOTS`：分市场定时槽位（`HH:MM|market[,market...]`，分号分隔）；非空时优先生效，触发时只分析选定市场个股，大盘复盘也跟随该槽位市场。Web 保存时会同步纯时间到 `SCHEDULE_TIMES`。
- `SCHEDULE_TIME`、`SCHEDULE_TIMES`：不是重启必需项。`SCHEDULE_SLOTS` 为空时，`SCHEDULE_TIMES` 为空再用 `SCHEDULE_TIME`；已运行的 scheduler 会按新槽位/时间重建 daily jobs。
